...
De Toolbox Continu Inzicht is in sprints ontwikkeld waarbij per keer één of meerdere modules zijn ontwikkeld.
...
1.  [Basis architectuur Toolbox Continu Inzicht](modules/architectuur.qmd)
    ...
    In de Toolbox Continu Inzicht is een belangrijk uitgangspunt dat verschillende bestandsformaten gebruikt kunnen worden.De functies zijn zo opgezet dat elk formaat als in- of uitvoer kan dienen.
...
6.  [Impactanalyse: effect nieuwe statistiek en rekenregels op de faalkans](modules/impactanalyse.qmd)
    ...
    Een fragility curve beschrijft de conditionele kans gegeven een belasting, zoals een waterstand. Om inzicht te krijgen in de faalkans per jaar moet de fragility curve gecombineerd worden met de kans van voorkomen van verschillende waterstanden (overschrijdingsfrequentielijn waterstanden). Dit maakt het mogelijk om het effect van veranderingen in de belastingstatistiek - bijvoorbeeld door klimaatverandering, een systeemmaatregel zoals rivierverruiming of nieuwe rekenregels - op de faalkans per jaar te vergelijken met een referentiesituatie. De duiding van de verandering van de faalkans is in klassen van grote kansafname tot grote kanstoename. Deze klassen zijn door de gebruiker zelf te definiëren.

7.  [Inspectieresultaten inlezen en weergeven](modules/inspectieresultaten.qmd)

    Het statusbeeld van de dijken op basis van fragility curves, eventueel bijgesteld met een beheerdersoordeel, is te verrijken met informatie uit de inspecties. Dit kunnen historische waarnemingen zijn, maar ook de voorjaarsinspectie of een melding tijdens een calamiteiteninspectie. Met de Toolbox Continu Inzicht is het mogelijk om de inspectiedata te filteren, classificeren en op de kaart te tonen in de viewer van de Toolbox Continu Inzicht.

8. [Maatregelen](modules/maatregelen.qmd)

    Tijdens crisissituaties kunnen maatregelen worden genomen om een dijk tijdelijk te versterken. In dat geval moet de fragility curve aangepast worden zodat het nog representatief is voor de gegeven situatie. Deze aangepastcurve kan uit een bibliotheek van bestaande curves worden gehaald of als deze niet beschikbaar is kan de curve aangepast worden door deze te verschuiven.

9. [Overstromingsrisico](modules/flood_scenario.qmd)

    Om inzicht te geven in de actuele risico kunnen overstromingsscenarios worden gebruik. Deze risico's kunnen helpen om prioriteiten te stellen tijdens hoogwater, waarbij er niet alleen een kans of een afzonderlijk gevolg wordt beschouwd. De gevolgen kunnen bijvoorbeeld worden uitgedrukt in economische schade, slachtoffers en getroffenen.

10. [GWDI (grondwaterdroogte-indicator)](modules/gwdi.qmd)

    Met GWDI wordt op basis van neerslag, verdamping, locatie-informatie, Pastas-modellen en referentiestatistiek een actuele droogte-indicator berekend per rastercel.
