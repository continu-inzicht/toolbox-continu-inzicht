...
Als de fragility curves van tevoren zijn berekend met een bepaalde maatregel, kunnen we de huidige curve vervangen met een voorberekende curve die past bij de maatregel.
Als de curve er niet is die past bij de maatregel, verschuiven we de originele curve. Hiervoor is ook een koppeling nodig tussen maatregel en effect.

De standaard manier is om voor alle dijkvakken en faalmechanismes in een keer alles aan te passen.

1. Voor alle dijkvakken en faalmechanismes in één keer alle fragility curves aanpassen met [`LoadCachedFragilityCurveMultiple`](#sec-LoadCachedFragilityCurveMultiple).
...
1. Voor een dijkvak, maar voor alle faalmechanismen in een keer de fragility curves aanpassen met [`LoadCachedFragilityCurve`](#sec-LoadCachedFragilityCurve).

1. De fragility curve kan per dijkvak aangepast worden voor één mechanisme met [`LoadCachedFragilityCurveOneFailureMechanism`](#sec-LoadCachedFragilityCurveOneFailureMechanism). Hierbij is de aanpassing het mees
...
### Voor alle vakken en alle faalmechanismes de aanpassingen maken {#sec-LoadCachedFragilityCurveMultiple}

Om voor alle vakken de fragility curve van alle faalmechanismes aan te passen kan de functie `LoadCachedFragilityCurveMultiple` worden gebruikt.
...
Hiervoor moet als derde een data adapter met `section_id`,`failuremechanism_id` en `measure_id` worden meegegeven.
...
Deze standaard waarde kan worden aangepast met `default_measure_id` in de `yaml`.
Omdat de 3 functies `LoadCachedFragilityCurve*` functionaliteit delen moet dit altijd onder het kopje van `LoadCachedFragilityCurve`.
...
### Per vak voor alle faalmechanismes de fragility curve aanpassen{#sec-LoadCachedFragilityCurve}

Om voor één vak de fragility curve van alle faalmechanismes aan te passen kan de functie `LoadCachedFragilityCurve` worden gebruikt.
...
De tweede data adapter moet de koppeling van `measure_id` met effect zijn, indien er geen voorberekende curve beschikbaar is, wordt de curve met deze hoeveelheid verschoven.
Om de aanpassing te doen moet een koppeling worden gemaakt tussen het faalmechanisme: (`failuremechanism_id`) en de maatregel  (`measure_id`), deze wordt als derde data adapter opgegeven.
...
Als laatste kan ook gekozen worden voor een eigen implementatie. Dit kan handig zijn als de keuze van de `measure_id` af hangt van een fysieke waarde: bijv. de windrichting. Hiervoor gebruik je de class `FragilityCurve`. _Let op: deze functionaliteit is voor de geavanceerde gebruiker_.
