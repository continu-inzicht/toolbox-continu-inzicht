...
- De faalmechanismes met de bijbehorende ids
- De kansen per sectie en faalmechanisme, en
- De koppeling van secties naar segmenten. <br>

De faalmechanismes worden gecombineerd naar een kans over alle secties (voorheen aangeduid als de ringkans) per faalmechanisme. Daarna worden de kansen over alle secties per faalmechanisme gecombineerd naar 1 kans over alle secties en over alle faalmechanismen. <br>
...
- De koppeling tussen secties en segementen, deze wordt ook gebruikt in `CalculateFloodScenarioProbability`.
- Het overstromingsrisico per gebied, uitgesplits per segement, dit is het resultaat van `CalculateFloodRisk`.

De output is een tabel met per gebied, per risicomaat het maatgevende vak. De risico maten moeten in de input van PostProcessFloodRisk worden opgegeven in de globalvariable `risk_metric_columns`. Deze zijn hetzelfde als de kolomnamen van de verschillende risicomaten in de output van `CalculateFloodRisk`. De naam van het maatgevende vak wordt teruggegeven in een nieuwe kolom met de naam `{risicomaat}_section_id`.
