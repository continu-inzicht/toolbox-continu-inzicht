...
Dit voorbeeld laat zien hoe met behulp van de `CalculateFloodScenarioProbability` scenariokansen berekend worden uit kansen per sectie en per faalmachanismen. <br>
Hiervoor zijn 3 data adapters nodig;
- De faalmechanismen met de bijbehorende ids
- De kansen per sectie en faalmechanisme, en
- De koppeling van secties naar segmenten. <br>

De faalmechanismen worden gecombineerd naar een kans over alle secties (voorheen aangeduid als de ringkans) per faalmechanisme. Daarna worden de kansen over alle secties per faalmechanisme gecombineerd naar 1 kans over alle secties en over alle faalmechanismen. <br>
...
De output bevat twee data adapters:
- De scenariokans per segment, deze kan gebruikt worden om een bijpassend scenario te selecteren
- De totaal gecombineerde faalkans van alle vakken. Deze kan gebruikt worden om het risico van een gebied te bepalen.
...
|affected_people_grid|affected_people|sum|
...
DataAdapter:
    ...
    output_flood_risk_results_per_segment:
        type: csv
        path: "hidden_rekentest_flood_risk_results_per_segment.csv"
...
## PostProcessFloodRisk{#sec-PostProcessFloodRisk}

Het resultaat van de overstromingsberekening kunnen we verder verwerken om het overstromingsrisico van een gebied te relateren aan het maatgevende vak. Hiermee wordt inzichtelijk welk vak kritiek is voor een bepaald gevolg.  We selecteren eerst het traject met de grootste bijdragen aan het risio van een gebied.
Omdat we het risico niet per vak berekenen, maar voor een traject, gebruiken we de faalkans van een vak. Hierbij worden de faalkansen van de verschillende faalmechanismen onafhankelijk gecombineerd tot een totale vakkans. Het vak met de hoogste faalkans wordt gekoppeld aan het gebied; dit kan dan eenvoudig getoond worden in een viewer.
...
Dit is bijna hetzelfde proces als in [CalculateFloodScenarioProbability](#sec-CalculateFloodScenarioProbability), maar dan vanuit risico terug naar de kans.

Voor de functie `PostProcessFloodRisk` zijn 3 data adapters nodig:
- De faalkans per segment en per faalmechanisme, deze wordt ook gebruikt in `CalculateFloodScenarioProbability`.
- De koppeling tussen secties en segmenten, deze wordt ook gebruikt in `CalculateFloodScenarioProbability`.
- Het overstromingsrisico per gebied, uitgesplitst per segement, dit is het resultaat van `CalculateFloodRisk`.

De output is een tabel met per gebied, per risicomaat het maatgevende vak. De risicomaten moeten in de input van `PostProcessFloodRisk` worden opgegeven in de variable `risk_metric_columns`. Deze zijn hetzelfde als de kolomnamen van de verschillende risicomaten in de output van `CalculateFloodRisk`. De naam van het maatgevende vak wordt teruggegeven in een nieuwe kolom met de naam `{risicomaat}_section_id`.
