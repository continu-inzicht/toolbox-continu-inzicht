...
De tijd waarop de berekening van de toolbox plaats vindt is ook te configureren met `calc_time`. Deze tijd moet in het ISO 8601 format zijn en wordt uitgelezen met [datetime.datetime.fromisoformat](https://docs.python.org/3/library/datetime.html#datetime.date.fromisoformat). Indien geen tijd wordt meegegeven, wordt maakt de `Config` klasse deze zelf aan met het moment van runnen. 
...
Naast data adapters definieren in het configuratie bestand, kan dit ook vanuit python zelf met de functie `set_dataframe_adapter`. 

Deze kan op twee manieren worden: 

1. De data adapter wordt van te voren in de configuratie opgegeven met het type python, de functie geeft de desbetrefende dataframe door.
1. Er is geen data adapter, deze wordt aangemaakt bij het runnen van de code. Let op: het configuratie bestand verandert niet mee. 
