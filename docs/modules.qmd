...
#### FragilityCurvePipingFixedWaterlevel{#sec-Bepalen-fragility-curves-STPH-single}
Een fragility curve kan berekend door de functie `FragilityCurvePipingFixedWaterlevel` aan te roepen. Hier zijn twee dataframes voor nodig: de eeste met informatie over de probabilitische berekening, de tweede met een reeks waterstanden waarvoor de fragility curve wordt afgeleid. Als opties kunnen debug en progress opties worden opgegeven in de `config.yaml`, standaard staan deze uit. De type functie die gebruikt wordt voor de grenstoestand (`z_type`) kan worden mee geven als optie, standaard is dit 'sellmeijer' maar kan ook aangepast worden naar 'heave', 'uplift' of 'combi'. Bij het combineren van wordt het minimum van de drie sub-mechanismes genomen in de grenstoestand functie.
...
De functie `FragilityCurvePipingFixedWaterlevelSimple` rekent met alle beschikbare grenstoestanden benoemd bij [`FragilityCurvePipingFixedWaterlevel`](#sec-Bepalen-fragility-curves-STPH-single) en geeft de curve gecombineerde terug. Het verschil zit in hoe de curves worden gecombineerd. Het combineren gebeurt hier door het minimum van de kansen per sub mechanisme te nemen. Dit is een simplere manier vergeleken met `FragilityCurvePipingFixedWaterlevel`. 
