...
from toolbox_continu_inzicht import BelastingFews
...
def test_create_params():
  ...
  t_now = datetime(2024,10,17,12,0,0,).replace(tzinfo=timezone.utc)
  params = fews.create_params(t_now=t_now, options=options, locations=locations)
  
  n_moments = len(options["moments"]) - 1
  starttime = t_now + timedelta(hours=int(options["moments"][0]))
  endtime = t_now + timedelta(hours=int(options["moments"][n_moments]))
...
def test_create_dataframe():
  ...
  t_now = datetime(2024,10,17,14,0,0,).replace(tzinfo=timezone.utc)
  ...
  df_out = fews.create_dataframe(options=options, t_now=t_now, json_data=json_data, locations=locations)
