...
def test_run_LoadsToMoments_tide_missing_moment():
    # zorg dat we een recent bestand hebben
    test_data_sets_path = Path(__file__).parent / "data_sets"
    config = Config(
        config_path=test_data_sets_path
        / "test_loads_to_moments_tide_config_missing.yaml"
    )
    config.lees_config()
...
def test_run_LoadsToMoments_tide_missing_moment_999():
    ...
    config = Config(
        config_path=test_data_sets_path
        / "test_loads_to_moments_tide_config_missing_999.yaml"
    )
    config.lees_config()
...
def test_run_LoadsToMoments_tide_missing_moment_middle():
    # zorg dat we een recent bestand hebben
    test_data_sets_path = Path(__file__).parent / "data_sets"
    config = Config(
        config_path=test_data_sets_path
        / "test_loads_to_moments_tide_config_missing_middle.yaml"
    )
    config.lees_config()
